...
# Target teams
TARGET_TEAMS = ["Bahman 1", "Bahman 2", "Bahman 3", "Bahman 4"]
...
# Fetch the HTML for all poules once to avoid redundant network requests
print("Fetching schedule data for all poules...")
poule_soups = []
for poule in POULES:
    response = requests.get(BASE_URL, params={'entry': 'speeldata', 'poule': poule})
    poule_soups.append(BeautifulSoup(response.text, 'html.parser'))

# Process each team
for target_team in TARGET_TEAMS:
    print(f"\nProcessing schedule for {target_team}...")
    
    cal = Calendar()
    cal.add('prodid', f'-//VC Geldrop Schedule {target_team}//mxm.dk//')
    cal.add('version', '2.0')
    
    events_added = 0
    
    for soup in poule_soups:
        ...
        for row in table.find_all('tr'):
            ...
            if not parts: 
                continue
            date_str = parts[-1]
            tijd = cols[1].get_text(strip=True)
            ...
            if target_team in thuis or target_team in gast:
                ...
                role = "referee"
                match_type = "Match" # Tricking Spond to select Home match
            else:
                continue
    ...
    # Write to file
    filename_team = target_team.replace(" ", "").lower() # e.g. bahman1
    output_file = f'{filename_team}_schedule.ics'
    with open(output_file, 'wb') as f:
        f.write(cal.to_ical())

    print(f"Successfully generated {output_file} with {events_added} events.")
